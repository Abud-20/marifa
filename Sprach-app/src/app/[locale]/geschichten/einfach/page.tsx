...
interface EinfachProps {
  params: Promise<{ locale: Locale }>;
}

export default function Einfach({ params }: EinfachProps) {
  ...
  if (!currentStoryData.title || !currentStoryData.paragraphs || !currentStoryData.questions) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-100 to-emerald-200 p-4 flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#7a9d54] mx-auto mb-4"></div>
          <h1 className="text-2xl font-bold text-[#523529]">Loading...</h1>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-100 via-emerald-100 to-teal-200 p-4 relative overflow-hidden">
      {/* Animated Background Elements */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-10 left-10 w-20 h-20 bg-green-300/20 rounded-full animate-pulse"></div>
        <div className="absolute top-32 right-20 w-16 h-16 bg-emerald-300/20 rounded-full animate-bounce"></div>
        <div className="absolute bottom-20 left-1/4 w-24 h-24 bg-teal-300/20 rounded-full animate-ping"></div>
        <div className="absolute bottom-40 right-1/3 w-12 h-12 bg-green-400/20 rounded-full animate-spin"></div>
      </div>

      <div className="max-w-4xl mx-auto relative z-10">
        {/* Header */}
        <div className="text-center mb-8">
          <h1 className="text-5xl font-bold text-[#523529] mb-4 bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent">
            {getTranslation(locale, 'stories', 'easyStories')}
          </h1>
        ...
        {/* Book Layout */}
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-2xl overflow-hidden border border-white/20" style={{ minHeight: '600px' }}>
          <div className="p-8">
            {/* Story Title */}
            <div className="text-center mb-8">
            ...
            <div className="space-y-6 text-lg leading-relaxed mb-8 max-h-96 overflow-y-auto pr-4 custom-scrollbar">
              {currentStoryData.paragraphs.map((paragraph: string, index: number) => (
                <p 
                  key={index} 
                  className="text-[#523529] text-justify transition-all duration-500 hover:bg-green-50/50 p-2 rounded-lg"
            ...
            {/* Test Section */}
            <div className="border-t-2 border-gradient-to-r from-green-200 to-emerald-200 pt-8">
              {!showTest ? (
                <div className="text-center">
              <button
                onClick={() => setShowTest(true)}
                    className="bg-gradient-to-r from-green-500 to-emerald-600 text-white px-10 py-4 rounded-xl font-bold text-xl hover:from-green-600 hover:to-emerald-700 transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl"
              >
            ...
            {!showResults ? (
              <div>
                      <h3 className="text-2xl font-bold text-[#523529] mb-6 text-center bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent">
                      ...
                      <div className="mb-8">
                        <p className="text-xl text-[#523529] mb-6 font-semibold bg-green-50 p-4 rounded-lg border-l-4 border-green-500">
                        ...
                        <div className="space-y-4">
                          {currentStoryData.questions[currentQuestion].options.map((option: string, index: number) => {
                            ...
                            if (showFeedback) {
                              ...
                              buttonClass += " bg-white hover:bg-gradient-to-r hover:from-green-50 hover:to-emerald-50 border-gray-200 hover:border-green-300 hover:shadow-md";
                            }
                      ...
                      >
                                <span className="inline-block w-8 h-8 bg-green-100 text-green-600 rounded-full text-center font-bold mr-3">
                      ...
                      <div className="text-center text-gray-600 mb-6 bg-green-50 p-4 rounded-lg">
              ...
              /* Results */
              <div className="text-center">
                      <h3 className="text-3xl font-bold text-[#523529] mb-6 bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent">
                        🏆 {getTranslation(locale, 'stories', 'testResult')}
                      </h3>
                
                      <div className="text-5xl font-bold bg-gradient-to-r from-green-500 to-emerald-600 bg-clip-text text-transparent mb-6">
                        {score} / {currentStoryData.questions.length}
                </div>
                
                      <p className="text-xl text-[#523529] mb-8 bg-gradient-to-r from-green-50 to-emerald-50 p-6 rounded-xl">
                        {score === currentStoryData.questions.length 
                  ...
                  <button
                    onClick={resetTest}
                          className="bg-gradient-to-r from-green-500 to-emerald-600 text-white px-8 py-4 rounded-xl font-bold hover:from-green-600 hover:to-emerald-700 transition-all duration-300 transform hover:scale-105 shadow-lg"
                  >
        ...
        <div className="text-center mt-8">
          <Link 
            ...
            className="inline-block bg-gradient-to-r from-green-500 to-emerald-600 text-white px-8 py-4 rounded-xl font-bold hover:from-green-600 hover:to-emerald-700 transition-all duration-300 transform hover:scale-105 shadow-lg"
          >
